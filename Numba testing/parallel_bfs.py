...
def bfs_parallel(graph, start_node):
    ...
    while not Q.empty():
        ...
        # only pass in the subgraphs and not the entire graph in each task
        subgraphs = {}
        for task in tasks:
            for node_id in task:
                if type(node_id) == int:    # if a processor is not getting a task, the only entry in `task` is None
                    subgraphs[node_id] = graph[node_id]

        # tasks = [(i, distances, graph, Q, mutex) for i in tasks]
        tasks = [(i, distances, subgraphs, Q, mutex) for i in tasks]
...
if __name__ == "__main__":
    runs = 3  # Number of runs for the BFS function
    main(runs=runs)